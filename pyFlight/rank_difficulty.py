            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if easyRect.collidepoint(mouse_pos):
                        resultEasy = result.GameResult(self.gamepad, 'RRRR', 0, "Easy")
                        resultEasy.show()
                    elif normalRect.collidepoint(mouse_pos):
                        resultNormal = result.GameResult(self.gamepad, 'RRRR', 0, "Normal")
                        resultNormal.show()
                    elif nightmareRect.collidepoint(mouse_pos):
                        resultNightmare = result.GameResult(self.gamepad, 'RRRR', 0, "Nightmare")
                        resultNightmare.show()
